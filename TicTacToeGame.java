...
public class TicTacToeGame {
    ...
    *  the index of the next move.
	...
	public TicTacToeGame(TicTacToeGame base, int next){
		...
		TicTacToeGame newbase = new TicTacToeGame();
		lines = base.lines;
		columns = base.columns;
		sizeWin= base.sizeWin;
		for(int i=0; i<base.board.length; i++){
			newbase.board[i]=base.board[i];
		}
		newbase.play(next);
    ...
    *  the TicTacToeGame instance to be compared with this one
	...
	public boolean equals(TicTacToeGame other) {
		...
		if (this.lines ==other.lines && this.columns==other.columns && this.sizeWin==other.sizeWin ){
		  for (int i=0; i<other.columns*other.lines; i++){
			if (this.board[i]!=other.board[i]){
				result= false;
			}
		  }
		  if ((this.level!=other.level)|| (this.gameState!=other.gameState) || result==false){
			result = false;
		  }
		}
		return result;
